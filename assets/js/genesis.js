...
jQuery(document).ready(function($) {
    ...
    if ($('body').not('.dvt-agent-iphone') && (/iPhone/i).test(navigator.userAgent || navigator.vendor || window.opera)) {
        $('body').addClass('dvt-agent-iphone');
    }
    if ($('body').not('.dvt-agent-ipad') && (/iPad/i).test(navigator.userAgent || navigator.vendor || window.opera)) {
        $('body').addClass('dvt-agent-ipad');
    }
    // Check for touch device
    if ($('body').not('.dvt-agent-touch') && ('ontouchstart' in window || navigator.msMaxTouchPoints)) {
        $('body').addClass('dvt-agent-touch');
    }
    ...
    if (navigator.userAgent.indexOf('MSIE')!==-1 || navigator.appVersion.indexOf('Trident/') > 0) {
        $('body').addClass('dvt-agent-explorer');
    }
    // Check for Edge
    if (window.navigator.userAgent.indexOf('Edge') > -1) {
        $('body').addClass('dvt-agent-edge');
    }

    /********************
     Object Fit Fix For IE in Card Images
    /*******************/
    $('body.dvt-agent-explorer, body.dvt-agent-edge').find('.card-horizontal .card-img').each(function() {
        var image = $(this);
    ...
    /*******************/
    $('.dvt-icon-set-expandable a').on('click', function(e) {
        if(!$(this).hasClass('dvt-icon-set-item-action')) {
            e.preventDefault();
            e.stopPropagation();
        }
        $(this).parent('.dvt-icon-set-expandable').toggleClass('open');
    });
...
jQuery(document).ready(function($) {
    ...
    /*******************/
    if (!$('body').hasClass('dvt-agent-touch') && $('.dvt-nav').hasClass('dvt-nav-hover')) {
        $('.dvt-nav-menu ul li').on('mouseover', function() {
            $(this).find('.dvt-nav-dropdown').removeClass('closed');
        }).on('mouseout', function() {
            $(this).find('.dvt-nav-dropdown').addClass('closed');
        });
    } else {
        $('.dvt-nav-menu ul li a:not(:only-child)').on('click', function(e) {
            $(this).siblings('.dvt-nav-dropdown').toggleClass('closed');
            $('.dvt-nav-dropdown').not($(this).siblings()).addClass('closed');
            e.stopPropagation();
            e.preventDefault();
        });
    }
    ...
    $('html').click(function() {
        $('.dvt-nav-dropdown').addClass('closed');
    });

    $('#nav-toggle').click(function(e) {
        $('.dvt-nav-menu .dvt-nav-list').slideToggle();
        $('.dvt-nav-menu .dvt-nav-widget').slideToggle();
        e.preventDefault();
    });
    ...
    /********************
     Sticky header
    /*******************/
    var $header = $('body').find('.dvt-nav-sticky');
    if($header.length > 0) {
        new Waypoint.Sticky({
            element: $header[0],
        ...
        new Waypoint({
            ...
            handler: function(direction) {
                if(direction === 'down') {
                    $header.addClass('dvt-nav-scrolled');
                } else {
                    $header.removeClass('dvt-nav-scrolled');
                }
            }
        });
    }
    ...
    new Waypoint({
        ...
        handler: function(direction) {
            if(direction === 'down') {
                $('.dvt-go-top').removeClass('hide');
            } else {
                $('.dvt-go-top').addClass('hide');
            }
        }
    });
    $('.dvt-go-top').click(function(event) {
...
jQuery(document).ready(function($) {
    ...
    function initParallax() {
        $('body:not(.dvt-agent-touch) .section-background').parallaxBackground({
...
jQuery(document).ready(function($) {
    ...
    $('.dvt-nav-smooth-scroll').click(function(event) {
        scrollToMenuLinkHash(event.target);
    });

    // Smooth scrolling from another page
    if (window.location.hash) {
        $('.dvt-nav-smooth-scroll').each(function(index, menuLink) {
